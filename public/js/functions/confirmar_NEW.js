...
btnAprobar.addEventListener("click", ()=>{confirmar("Confirmado")})
...
function confirmar(status){
  ...
  axios({
    method: 'post',
    url: `/confirmar_solicitud`,
    data: JSON.stringify(data),
    headers: { 'content-type': 'application/json' }
  })
    .then((result) => {
      
      window.location = `/confirmar_list/${status}`
...
//             if (status == 'Rechazado') { icon = `<span class="icoSidebar fas fa-user-times text-danger" onclick="historial()"></span>`, window.location = `/confirmar_list/sidebar` } else
...
function getTableInfo() { 
  ...
  axios({
    ...
    .then((result) => {
      ...
      let fechaCreacion= result.data.result[0][0].fecha_solicitud
      inputFechaCreacion.value=fechaCreacion
      let username= result.data.result[5]
      ...
      if((jefe_nombre[i].toUpperCase() === username.toUpperCase()) && status != "Pendiente"){
        ...
        if (status == 'Pendiente') {temp.push( icon = `<span class="icoSidebar fas fa-user-clock text-secondary" onclick="historial()"></span>`) } else
          if (status == 'Confirmado') {temp.push( icon = `<span class="icoSidebar fas fa-user-plus text-info" onclick="historial()"></span>`)} else
            if (status == 'Rechazado') {temp.push( icon = `<span class="icoSidebar fas fa-user-times text-danger" onclick="historial()"></span>` ), window.location = `/confirmar_list/sidebar`} else
              if (status == 'Aprobado') {temp.push( icon = `<span class="icoSidebar fas fa-user-check text-primary" onclick="historial()"></span>`) } else
                if (status == 'Finalizado') {temp.push( icon = `<span class="icoSidebar fas fa-user-tie text-success" onclick="historial()"></span>` )}
