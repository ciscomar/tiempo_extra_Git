...
function getTableInfo() { 
  ...
  data = { "id": `${id}`, "paso": "historial_confirmar"}
  axios({
    method: 'post',
    url: `/solicitud_historial_id`,
    data: JSON.stringify(data),
    ...
    .then((result) => {
      ...
      for (let i = 0; i < empleados.length; i++) {
        ...
        if (status == 'Pendiente') {temp.push( icon = `<span class="icoSidebar fas fa-user-clock text-secondary" onclick="historial()"></span>`) } else
          if (status == 'Confirmado') {temp.push( icon = `<span class="icoSidebar fas fa-user-plus text-info" onclick="historial()"></span>`); btnCancelar.hidden=false} else
            if (status == 'Rechazado') {temp.push( icon = `<span class="icoSidebar fas fa-user-times text-danger" onclick="historial()"></span>`);  btnEditar.hidden=false } else
