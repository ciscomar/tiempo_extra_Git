...
btnAprobar.addEventListener("click", ()=>{finalizar("Finalizado")})
...
function finalizar(status){
  ...
  axios({
    method: 'post',
    url: `/finalizar_solicitud`,
    data: JSON.stringify(data),
    headers: { 'content-type': 'application/json' }
  })
    .then((result) => {
      
      window.location = `/finalizar_list/${status}`
...
function getOtherInfo(fecha_start, fecha_end) { 
  ...
  //       if (status == 'Pendiente') { icon = `<span class="icoSidebar fas fa-user-clock text-secondary" onclick="historial()"></span>`, window.location = `/finalizar_list/sidebar` } else
  //         if (status == 'Confirmado') { icon = `<span class="icoSidebar fas fa-user-plus text-info" onclick="historial()"></span>`, window.location = `/finalizar_list/sidebar` } else
  //           if (status == 'Rechazado') { icon = `<span class="icoSidebar fas fa-user-times text-danger" onclick="historial()"></span>` , window.location = `/finalizar_list/sidebar`} else
  //             if (status == 'Aprobado') { icon = `<span class="icoSidebar fas fa-user-check text-primary" onclick="historial()"></span>` } else
  //               if (status == 'Finalizado') { icon = `<span class="icoSidebar fas fa-user-tie text-success" onclick="historial()"></span>` , window.location = `/finalizar_list/sidebar`}
  ...
  axios({
    ...
    .then((result) => {
      ...
      for (let i = 0; i < empleados.length; i++) {
        ...
        if (status == 'Pendiente') {temp.push( icon = `<span class="icoSidebar fas fa-user-clock text-secondary" onclick="historial()"></span>`), window.location = `/finalizar_list/sidebar` } else
          if (status == 'Confirmado') {temp.push( icon = `<span class="icoSidebar fas fa-user-plus text-info" onclick="historial()"></span>`), window.location = `/finalizar_list/sidebar` } else
            if (status == 'Rechazado') {temp.push( icon = `<span class="icoSidebar fas fa-user-times text-danger" onclick="historial()"></span>` ), window.location = `/finalizar_list/sidebar`} else
              if (status == 'Aprobado') {temp.push( icon = `<span class="icoSidebar fas fa-user-check text-primary" onclick="historial()"></span>`) } else
                if (status == 'Finalizado') {temp.push( icon = `<span class="icoSidebar fas fa-user-tie text-success" onclick="historial()"></span>` ), window.location = `/finalizar_list/sidebar`}
